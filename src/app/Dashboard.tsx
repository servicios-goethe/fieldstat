...
export function Dashboard({ email, isPrincipal, onSignOut }: { email: string; isPrincipal: boolean; onSignOut: () => void }) {
  ...
  return <main className="dashboard"><header><div><p className="eyebrow">Goethe Schule</p><h1>FieldStats</h1><p className="subtitle">{email}</p></div><button className="secondary" onClick={onSignOut}>Cerrar sesión</button></header><nav className="menu" aria-label="Navegación principal"><button className={section === 'inicio' ? 'selected' : 'secondary'} onClick={() => setSection('inicio')}>Inicio</button>{isPrincipal && <button className={section === 'configuracion' ? 'selected' : 'secondary'} onClick={() => setSection('configuracion')}>Configuración</button>}</nav>{section === 'inicio' && <div className="panels"><section className="panel"><h2>Inicio</h2><p>Gestioná planteles, jugadores y partidos desde este panel.</p></section><PlantelesJugadores /><InscripcionExistente /><Asistencia /><Partidos /><Convocatorias /><MisConvocatorias /></div>}{section === 'configuracion' && isPrincipal && <div className="panels"><Usuarios /><Categorias /><Deportes /><Temporadas /><Sedes /><Equipos /></div>}</main>
}
